...
#+BEGIN_SRC emacs-lisp
  (defun tom/build-tunnel-args (args)
    ...
    (let ((arglist '("-v")))
      (progn
        (add-to-list 'arglist "-N" t)
        (add-to-list 'arglist "-L" t)
        (add-to-list 'arglist
                     (concat (tom/assoc :localport args)
                             ":"
                             (tom/assoc :tunnel-ip args)
                             ":"
                             (tom/assoc :tunnel-port args)) t)
        (add-to-list 'arglist "-l" t)
        (add-to-list 'arglist (tom/assoc :user args) t)
        (add-to-list 'arglist "-p" t)
        (add-to-list 'arglist (tom/assoc :port args) t)
        (add-to-list 'arglist (tom/assoc :host args) t)
        arglist)))
#+END_SRC
