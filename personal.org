...
#+BEGIN_SRC emacs-lisp
  ...
  (push '("\*GEBEN<.*> context\*" :regexp t :position left :width 0.3 :stick t :dedicated t) popwin:special-display-config)
  (push '("\*GEBEN<.*> output\*" :regexp t :position left :width 0.3 :stick t) popwin:special-display-config)
#+END_SRC
#+BEGIN_SRC emacs-lisp
