...
#+BEGIN_SRC emacs-lisp
  (defun tom/reloadgnus ()
    ...
    (interactive)
    (let ((gnusfiles '(gnus gnus-agent gnus-art gnus-async gnus-bcklg
                            gnus-cache gnus-cite gnus-ems gnus-group gnus-int gnus-load
                            gnus-ml gnus-msg gnus-range gnus-score gnus-spec gnus-srvr
                            gnus-start gnus-sum gnus-undo gnus-util gnus-win)))
      (cl-loop for lib in gnusfiles do
               (ignore-errors (call-interactively (unload-feature lib))))
      (tom/gnus)))
#+END_SRC
* org-mode
