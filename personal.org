...
#+BEGIN_SRC emacs-lisp
  ...
  (spaceline-define-segment circe-tracking
    ...
    (when (and active (not (s-blank-str-p tracking-mode-line-buffers)))
      (s-trim (powerline-raw tracking-mode-line-buffers)))
    :fallback ".oO(...)"
    :enable t)
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-hook 'circe-query-mode-hook 'tom/circe-query-hook)
