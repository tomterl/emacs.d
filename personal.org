...
** Optimization
   #+BEGIN_SRC emacs-lisp
     ...
     (defvar tom/gc-cons-threshold (* 4 1024 1024) "Don't make this too big")
...
** `vterm`
  #+BEGIN_SRC emacs-lisp
    ...
    (let ((proj (if (projectile-project-p) (substring (projectile-project-root) 0 -1)))
          (projectile-switch-project-action '(lambda () (projectile-run-vterm))))
      (select-frame tom/shellframe)
      (raise-frame)
      (message (concat "Project " proj))
      (projectile-switch-project-by-name (file-name-nondirectory proj))))
...
** Flyspell: Change dictionary; key-bindings
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (setq lyspell-correct-interface 'flyspell-correct-ivy)
      (global-set-key
       (kbd "H-s")
...
*** Languagues embedded in `php-mode`-buffers
    #+BEGIN_SRC emacs-lisp
      ...
      (define-innermode poly-php-aql-innermode
        :mode 'aql-mode
        :head-matcher "^[ \t]*protected static $[a-z]* = <<<'?AQL'?\n"
        :tail-matcher "^AQL;\n"
        :head-mode 'host
        :tail-mode 'host)
