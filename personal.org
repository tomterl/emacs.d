...
#+BEGIN_SRC emacs-lisp
  ...
  (setq company-box-icons-php
     '((fa_tag :face font-lock-function-name-face) ;; Function
       (fa_tags :face font-lock-function-name-face) ;; Methods
       (fa_cog :face font-lock-variable-name-face) ;; Variable
       (fa_cube :face font-lock-constant-face) ;; Feature
...
*** circe spaceline segment
#+BEGIN_SRC emacs-lisp
  (require 'spaceline)
  (spaceline-define-segment circe-tracking
    "Display circe tracking buffers"
    (when (listp tracking-mode-line-buffers)
      (apply 'concat (cl-loop for entry in tracking-mode-line-buffers
                              collect (or (plist-get entry :propertize) ""))))
    :fallback ".oO(...)"
    :enable t)
#+END_SRC
*** persp-mode spaceline segment
#+BEGIN_SRC emacs-lisp
  (require 'spaceline)
  (spaceline-define-segment perspective-name
    "Display persp-mode acive perspective"
...
#+BEGIN_SRC emacs-lisp
  ...
  (spaceline-spacemacs-theme 'circe-tracking 'perspective-name)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun tom/circe-query-hook ()
    (let ((-buff (current-buffer)))
      (with-perspective "chat"
        (persp-add-buffer -buff))))
  (add-hook 'circe-mode-hook 'tom/circe-query-hook) 
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun tom/pomidor ()
    "Ensure pomidor is started in persp none"
    (interactive)
    (let ((buff (current-buffer))
          (pers (get-current-persp)))
      (pomidor)
      (persp-switch "none")
      (let ((pom-buff (get-buffer "*pomidor*")))
        (persp-remove-buffer pom-buff pers)
        (switch-to-buffer pom-buff)
        (local-set-key (kbd "q") `(lambda ()
                                    ""
                                    (interactive)
                                    (progn
                                      (persp-switch (persp-name ,pers))
