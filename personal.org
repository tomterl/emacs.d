...
Notify me when tracked buffers have activity -- but not more than a certain amount of time.
...
#+BEGIN_SRC emacs-lisp
  ...
  (defvar tom/chatnotifyintervall 30
    "Minimal delay between chat activity notifications in seconds")
  ...
  (defadvice tracking-add-buffer (after tracking-desktop-notify activate)
    (let ((current-t (float-time)))
      ...
      (if (> (- current-t tom/lastchatnotification) tom/chatnotifyintervall)
          (progn
            ...
            (setq  tom/lastchatnotification current-t
                   tom/chatnotification (notifications-notify :title "Emacs Active Buffers"
                                                              :body (s-replace-all '(("&" . "")("@" . "-at-")) (s-join "\n" tracking-buffers))
                                                              :timeout 750
